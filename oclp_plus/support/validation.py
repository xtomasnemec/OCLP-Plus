...
class PatcherValidation:
    ...
    def _validate_root_patch_files(self, major_kernel: int, minor_kernel: int) -> None:
        ...
        if not sys_patch_helpers.SysPatchHelpers(self.constants).generate_patchset_plist(patchset, f"OCLP-Plus-{major_kernel}.{minor_kernel}.plist", None, None):
            raise Exception("Failed to generate patchset plist")

        # Remove the plist file after validation
        Path(self.constants.payload_path / f"OCLP-Plus-{major_kernel}.{minor_kernel}.plist").unlink()
