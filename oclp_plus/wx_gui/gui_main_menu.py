...
class MainFrame(wx.Frame):
    ...
    def _generate_elements(self) -> None:
        ...
        # Title label: OCLP-Plus v{X.Y.Z}
        title_label = wx.StaticText(self, label=f"OCLP-Plus", pos=(-1, 128))
    ...
    def _preflight_checks(self):
        ...
        if "--update_installed" in sys.argv and self.constants.has_checked_updates is False and gui_support.CheckProperties(self.constants).host_can_build():
            ...
            pop_up = wx.MessageDialog(
                self,
                f"OCLP-Plus has been updated to the latest version: {self.constants.patcher_version}\n\nWould you like to update OpenCore and your root volume patches?",
                "Update successful!",
...
Please check the Github page for more information about this release."""
        ...
        sizer.AddSpacer(10)
        self.title_text = wx.StaticText(panel, label="A new version of OCLP-Plus is available!")
        self.description = wx.StaticText(panel, label=f"OCLP-Plus {oclp_version} is now available - You have {self.constants.patcher_version}{' (Nightly)' if not self.constants.commit_info[0].startswith('refs/tags') else ''}. Would you like to update?")
