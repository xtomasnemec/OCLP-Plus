...
class InitializeLoggingSupport:
    ...
    def __init__(self, global_constants: constants.Constants) -> None:
        ...
        self.log_filename: str  = f"Skyfall_{self.constants.patcher_version}_{log_time}.log"
    ...
    def _initialize_logging_path(self) -> None:
        ...
        if os.name == "nt":
            ...
            base_path = base_root / "Skyfall" / "Logs"
            try:
    ...
    def _start_logging(self):
        ...
        str_msg = f"# Skyfall ({self.constants.patcher_version}) #"
        str_len = len(str_msg)
    ...
    def _implement_custom_traceback_handler(self) -> None:
        ...
        def custom_excepthook(type, value, tb) -> None:
            ...
            error_msg = f"Skyfall encountered the following internal error:\n\n"
            ...
            try:
                result = applescript.AppleScript(f'display dialog "{error_msg}" with title "Skyfall ({self.constants.patcher_version})" buttons {{"Yes", "No"}} default button "Yes" with icon caution').run()
            except Exception as e:
