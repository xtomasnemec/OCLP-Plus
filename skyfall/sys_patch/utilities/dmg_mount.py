...
class PatcherSupportPkgMount:
    ...
    def _request_decryption_key(self, attempt: int) -> str:
        ...
        try:
            password = applescript.AppleScript(
                f"""
                set theResult to display dialog "{msg}" default answer "" with hidden answer with title "Skyfall" with icon file "{self.icon_path}"
    ...
    def _display_authentication_error(self) -> None:
        ...
        try:
            applescript.AppleScript(
                f"""
                display dialog "Failed to mount DortaniaInternal resources, please file an internal radar." with title "Skyfall" with icon file "{self.icon_path}"
                """
            ).run()
        except Exception as e:
            pass
    ...
    def _display_too_many_attempts(self) -> None:
        ...
        try:
            applescript.AppleScript(
                f"""
                display dialog "Failed to mount DortaniaInternal resources, too many incorrect passwords. If this continues with the correct decryption key, please file an internal radar." with title "Skyfall" with icon file "{self.icon_path}"
                """
            ).run()
        except Exception as e:
            pass
