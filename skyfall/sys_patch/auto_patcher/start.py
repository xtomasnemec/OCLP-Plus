...
class StartAutomaticPatching:
    ...
    def start_auto_patch(self):
        ...
        if dict:
            ...
            app = wx.App()
            mainframe = wx.Frame(None, -1, "Skyfall")
...
Please check the Github page for more information about this release."""
            ...
            sizer.AddSpacer(10)
            self.title_text = wx.StaticText(panel, label="A new version of Skyfall is available!")
            self.description = wx.StaticText(panel, label=f"Skyfall {version} is now available - You have {self.constants.patcher_version}{' (Nightly)' if not self.constants.commit_info[0].startswith('refs/tags') else ''}. Would you like to update?")
...
'''
        ...
        if utilities.check_seal() is True:
            ...
            if patches:
                ...
                if network_handler.NetworkUtilities("https://api.github.com/repos/dortania/OpenCore-Legacy-Patcher/releases/latest").verify_network_connection() is False:
                    warning_str = f"""\n\nWARNING: We're unable to verify whether there are any new releases of Skyfall on Github. Be aware that you may be using an outdated version for this OS. If you're unsure, verify on Github that Skyfall {self.constants.patcher_version} is the latest official release"""

                args = [
                    "/usr/bin/osascript",
                    "-e",
                    f"""display dialog "Skyfall has detected you're running without Root Patches, and would like to install them.\n\nmacOS wipes all root patches during OS installs and updates, so they need to be reinstalled.\n\nFollowing Patches have been detected for your system: \n{patch_string}\nWould you like to apply these patches?{warning_str}" """
    ...
    def _determine_if_versions_match(self):
        ...
        args = [
            "/usr/bin/osascript",
            "-e",
            f"""display dialog "Skyfall has detected that you are booting {'a different' if self.constants.special_build else 'an outdated'} OpenCore build\n- Booted: {self.constants.computer.oclp_version}\n- Installed: {self.constants.patcher_version}\n\nWould you like to update the OpenCore bootloader?" """
            f'with icon POSIX file "{self.constants.app_icon_path}"',
        ]
        output = subprocess.run(
            args,
    ...
    def _determine_if_boot_matches(self):
        ...
        try:
            ...
            args = [
                "/usr/bin/osascript",
                "-e",
                f"""display dialog "Skyfall has detected that you are booting OpenCore from an USB or External drive.\n\nIf you would like to boot your Mac normally without a USB drive plugged in, you can install OpenCore to the internal hard drive.\n\nWould you like to launch Skyfall and install to disk?" """
                f'with icon POSIX file "{self.constants.app_icon_path}"',
            ]
            output = subprocess.run(
                args,
