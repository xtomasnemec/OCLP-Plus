# Skyfall Privileged Helper Tool

`com.xtomasnemec.skyfall.priviledged-helper` is Skyfall's Privileged Helper Tool.

The architecture is as such:
1. The main application (Skyfall.app) will send arguments to the privileged helper tool to execute.
...
Since running Skyfall from source will lack Dortania's code signature, you will need to disable code signature verification in the privileged helper tool otherwise root commands will fail.
...
* Additionally you will be required to compile Skyfall.app with your own Developer ID Application certificate

If this is not possible, we recommend using [Skyfall's prebuilt binaries](../../SOURCE.md) instead.